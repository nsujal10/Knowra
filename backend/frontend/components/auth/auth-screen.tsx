...
export function AuthScreen({ initialTab = "signin" }: AuthScreenProps) {
  ...
  // Keep tab state synchronized if navigated via router or back button
  useEffect(() => {
    setTab(initialTab);
    clearErrors();
  }, [initialTab]);
  ...
  const switchTab = (nextTab: "signin" | "signup") => {
    clearErrors();
    setTab(nextTab);
    const destination = nextTab === "signin" ? "/login" : "/register";
    const queryString = rawRedirect ? `?redirect=${encodeURIComponent(rawRedirect)}` : "";
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <SSOButtons redirectTarget={redirectTarget} />
